...
export const searchPublication = async (req, res) => {
  const searchInput = "%" + req.body.description + "%";
...
export const filterPublications = async (req, res) => {
  ...
  const [rows] = await pool.query(
    "SELECT * FROM publication WHERE (eslora >= ? AND eslora <= ?) AND (manga >= ? AND manga <= ?) AND (idPuerto >= ? AND idPuerto <= ?)",
    [MinEslora, MaxEslora, MinManga, MaxManga, MinNumPuerto, MaxNumPuerto]
  );
...
export const orderPublications = async (req, res) => {
  ...
  const query = `SELECT * FROM publication ORDER BY ${orden} ${tipo}`;
...
export const renderFavourite = async (req, res) => {
  ...
  if (user.length > 0) {
    ...
    if (favoriteIds.length > 0) {
      ...
      publications = await pool.query("SELECT * FROM publication WHERE deleted = ?", ["F"]);
    }
